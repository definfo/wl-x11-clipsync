...
#! nix-shell -i runghc -p "haskellPackages.ghcWithPackages (hspkgs: [ hspkgs.utf8-string ])" wl-clipboard xclip clipnotify
...
import Data.Text (Text, dropWhileEnd, isPrefixOf, pack, strip)
...
decode = dropWhileEnd (== '\0') . decodeUtf8With lenientDecode
...
isTextMime text =
    pack "text/" `isPrefixOf` text
        || text == pack "UTF8_STRING"
        || text == pack "STRING"
...
getTargets server = do
    ...
    result <- try @IOException $ readCreateProcessWithExitCode p ""
    case result of
        Left _ -> return []
        Right (exitCode, stdout, _) ->
            case exitCode of
                ExitSuccess -> return $ T.lines . decode . fromString $ stdout
                _ -> return []
...
getMime server = do
  ...
  where
    ...
    pickMime targets
        -- 1) text/uri-list
        | pack "text/uri-list" `elem` targets = pack "text/uri-list"
        -- 2) text/html
        | pack "text/html" `elem` targets = pack "text/html"
        -- 3) image/*
        | Just imageMime <- findImageMime targets = imageMime
        -- 4) text/plain;charset=utf-8
        | pack "text/plain;charset=utf-8" `elem` targets = pack "text/plain;charset=utf-8"
        -- 5) text/plain
        | pack "text/plain" `elem` targets = pack "text/plain"
        -- 6) fallback
        | pack "UTF8_STRING" `elem` targets = pack "UTF8_STRING"
        -- default fallback
        | otherwise = pack "text/plain;charset=utf-8"

    findImageMime :: [Text] -> Maybe Text
    findImageMime [] = Nothing
    findImageMime (t : ts)
        | pack "image/" `isPrefixOf` t = Just t
...
unwrapResult :: forall a. DisplayServer -> ClipMethod -> Either IOException (ExitCode, String, String) -> IO a -> (String -> IO a) -> IO a
unwrapResult server cm result retf rets =
    case result of
        Left ex -> do
            putStrLn $ "Error " ++ show cm ++ " " ++ show server ++ " clipboard: " ++ show ex
            retf
        Right (exitCode, stdout, _) ->
            case exitCode of
                ...
                _ -> do
                    case server of
                        Wayland -> putStrLn $ "Error " ++ show cm ++ " " ++ show server ++ " clipboard: " ++ T.unpack (getClipName server cm) ++ " exited with non-zero status: " ++ show exitCode
                        -- NOTE:
                        -- xclip exited with 1 when history is empty
                        -- thus we should not handle this on X11 side
                        X11 -> return ()
                    retf
...
getClip server = do
    ...
    unwrapResult
        ...
        Get
        result
        (return (fromString "", pack ""))
        (\stdout -> return (fromString stdout, mime))
...
setClip server rawData mime = do
    let chosenMime =
            if server == Wayland && isTextMime mime
                then pack "text/plain;charset=utf-8"
                else mime
    let p = case server of
            Wayland -> proc "wl-copy" ["-t", T.unpack chosenMime]
            X11 -> proc "xclip" ["-selection", "clipboard", "-i", "-t", T.unpack chosenMime]
    result <-
        try @IOException $
            readCreateProcessWithExitCode p (toString rawData)
    unwrapResult
        server
        Set
        result
...
loop (lastWData, lastWMime) (lastXData, lastXMime) = do
    -- clipnotify will wake up on any clipboard change
    _ <- readCreateProcessWithExitCode (proc "clipnotify" []) ""

    -- Read both clipboards
    (wRaw, wMime) <- getWlClip
    (xRaw, xMime) <- getX11Clip

    -- Normalize text data to reduce duplicates
    let wNorm =
            if isTextMime wMime && wRaw /= fromString ""
                then normalize wRaw
                else wRaw

    let xNorm =
            if isTextMime xMime && xRaw /= fromString ""
                then normalize xRaw
                else xRaw

    let wChanged = wNorm /= fromString "" && wNorm /= lastWData
        xChanged = xNorm /= fromString "" && xNorm /= lastXData

    case (wChanged, xChanged) of
        (True, False) -> do
            -- Wayland changed
            when (wNorm /= lastXData) $ do
                putStrLn $ "[Wayland -> X11] MIME=" ++ T.unpack wMime
                setX11Clip wNorm wMime
            loop (wNorm, wMime) (wNorm, wMime)
        (False, True) -> do
            -- X11 changed
            when (xNorm /= lastWData) $ do
        ...
        (True, True) -> do
            ...
            putStrLn $ "[Conflict] Both changed. Preferring Wayland -> X11 (MIME=" ++ T.unpack wMime ++ ")"
            when (wNorm /= lastXData) $ do
                setX11Clip wNorm wMime
            loop (wNorm, wMime) (wNorm, wMime)
        (False, False) -> do
            -- No changes, keep previous state
            loop (lastWData, lastWMime) (lastXData, lastXMime)

run :: IO ()
run = do
    putStrLn "Starting Wayland ↔ X11 clipboard sync..."
    -- Initialize with current clipboard contents to avoid initial sync
    (wData, wMime) <- getWlClip
    (xData, xMime) <- getX11Clip
    loop (wData, wMime) (xData, xMime)
